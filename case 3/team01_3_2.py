...
for d in range(D):
    for t in range(T-1):
        for c in range(C):
            model.addConstr(x[d, t+1, c, 6] == x[d, t, c, 14], 'c4_%d_%d_%d' % (d, t, c)) 
...
for c in range(C):
    model.addConstr(gp.quicksum(y[d, t, c] for d in range(D) for t in range(T) if t not in [3] ) == 2, 'c5.3')
